				   ...
				   u32 value)
{
	...
	/* Do not validate and map the interrupt while we are masked. */
	...
	result = vtd_map_interrupt(cell, cell->ioapic_id,
				   (reg - IOAPIC_REDIR_TBL_START) / 2,
				   irq_msg);
	// HACK for QEMU
	if (result == -ENOSYS) {
