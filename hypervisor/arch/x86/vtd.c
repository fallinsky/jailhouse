...
static void vtd_flush_unit_cache(void *reg_base, void *inv_queue,
				 struct vtd_entry inv_request)
{
	...
	index = mmio_read64_field(reg_base + VTD_IQT_REG, VTD_IQT_QH_MASK);

	index = inv_queue_write(inv_queue, index, inv_request);
	index = inv_queue_write(inv_queue, index, inv_wait);

	mmio_write64_field(reg_base + VTD_IQT_REG, VTD_IQT_QH_MASK, index);
